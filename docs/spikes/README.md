...
- [entitlements-live.md](entitlements-live.md) — spike W4b (Track W): a hand-built Loopback driver and an entitlement authored as-code deployed to idm254, granted by an authored workflow (`Entitlement_Grant` in the engine log, `DirXML-EntitlementRef` on the recipient); four deploy/flow fixes found; the EntitlementConfiguration resource is a follow-up.
- [`prd-rest-live.md`](prd-rest-live.md) — start a PRD over REST (`/requests/permissions/v2`), approve, history; the permission-index delay; reference `docs/idapps-rest.md`
