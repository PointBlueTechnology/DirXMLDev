...
| driver icon added / changed | the `icon` `CHeavyData` attribute on the driver's CObject and the sibling `<ID>_icon.<ext>` that holds the bytes, moved together; a format change (`gif` → `png`) replaces the old file. New bytes in the same format leave the CObject alone. A tree without an icon leaves the project's alone (never a removal). The vault side is `DirXML-DriverImage`, one attribute write and no restart (`docs/designer-new-project.md` §7.2e) |
