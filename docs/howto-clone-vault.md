...
- The **DirXMLDev** repository with the engine jars copied into a real `lib/`
  directory (not symlinked), built once (`mvn -q test`). `bin/idm` then runs
  everything; on Windows use
