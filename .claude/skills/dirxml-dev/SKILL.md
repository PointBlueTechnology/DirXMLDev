...
| the Identity Applications | a form or workflow must be proven at runtime | `<env>.formsUrl`/`appsUser`/`appsPassword` in `environments.properties` for `bin/apps` (request, tasks, approve, history over REST — `docs/idapps-rest.md`), and — for the engine log — `kubectl`/SSH to the applications host; the browser (`https://host/idmdash`, a user with rights to the PRD) is only needed to *see* a form |
