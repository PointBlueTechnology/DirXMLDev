...
## Workflow (flow.*; Track W steps W2/W3, `docs/workflows.md`)
...
- `flow.activity.add <tree> --prd P --kind approval|condition|log|notification|mapping|provision|rest|role-request|resource-request|start-flow --id X --after Y [--via T] [--to Z] [--on-denied ID] [--on-false ID] [--name TEXT] [--addressee EXPR] [--timeout MS] [--ontimeout T] [--expression E] [--message M] [--template DN] [--entitlement-dn DN] [--entitlement-param V] [--form F] [--status approved|denied] [--driver D]` plus the kind-specific flags below — inserts activity `X` after `Y` (consuming `Y`'s single/`--via`-picked outgoing link, or — when `Y` is a branch — adding a new parallel leg to `--to`), wiring `X`'s default outgoing link(s) by kind (approval: `approved` to the successor, `denied` to the process's "Workflow Status Denied" mapping — created as `status_denied` → finish when absent — so Request History reads Denied; `--on-denied` overrides; condition: `true`/`false`; log/mapping/provision/notification/rest/role-request/resource-request/start-flow: `forward`) and its stock defaults (see below). `--kind branch` is refused — use `flow.branch.add`. `--kind mapping --status approved|denied` creates a status mapping (sets `flowdata.IDM_COMPLETED_APPROVAL_STATUS`). An approval binds an approval form (`--form F`, default the driver's stock `Approval Form` if present) with the stock data items — without one the Identity Applications cannot open the task. All four kinds below also get an empty `data-items` block, same as approval/mapping (data flows in/out through the usual `prd.map`/`flow.data.set`):
  - **rest**: `--protocol http|https --host H --port N --path P --method GET|POST|PUT|DELETE|PATCH` (all required; `--method` checked case-insensitively, stored as given) `[--content EXPR] [--header Key=Value]…` (repeatable — each becomes its own `<http-headers key="Key">Value</http-headers>` element; yes, each header element is itself named `http-headers`, the engine's JAXB binding names it that way) `[--timeout MS] [--content-type T] [--accept T] [--authorization H] [--status-to ITEM] [--content-to ITEM] [--content-type-to ITEM]`.
  - **role-request**: `--role EXPR`… (repeatable, ≥1 required) `--target EXPR`… (repeatable, ≥1 required) `[--target-type USER|GROUP|CONTAINER|CONTAINER_WITH_SUBTREE|ROLE] [--action GRANT|REVOKE|EXTEND] --description EXPR [--effective-date E] [--expiration-date E] [--correlation-id E]`.
  - **resource-request**: `--resource EXPR --target EXPR`… (repeatable, ≥1 required) `[--action GRANT|REVOKE] --description EXPR [--param source=target]…` (repeatable, one `<target-param source="…" target="…"/>` each) `[--correlation-id E]`.
  - **start-flow**: `--process EXPR --recipient EXPR`… (repeatable, ≥1 required) `[--correlation-id E]`. `--process` is typically a quoted PRD name/DN literal; `validate` warns (`flow-start-flow-unknown`) when it names no PRD found in this tree by name or DN.
- `flow.activity.set <tree> --prd P --id X [--name TEXT] [--attr name=value]… [--addressee EXPR] [--timeout MS] [--ontimeout T] [--expression E] [--message M] [--template DN] [--entitlement-dn DN] [--entitlement-param V] [--approver-type T] [--form F] [--driver D]` plus the same kind-specific flags as `flow.activity.add` (refused when they don't match the activity's own kind) — sets only what is given; `--form` (approval only) binds/replaces the approval form; `--attr activity-id` is refused (use `flow.activity.rename`); `--entitlement-dn`/`--entitlement-param` only apply to a provision activity; a repeatable flag (`--header`/`--role`/`--target`/`--param`/`--recipient`) replaces the whole list when given, leaves it alone when not.
...
- `export-project <tree> <projectDir> [--dry-run] [--json]` — update an existing Designer project in place from the tree (only the files the diff needs; refuses packaged new drivers and driver deletes; also carries a driver's forms/PRDs, refusing only that driver's provisioning when the project has no AppConfig for it, and a driver icon the tree added/changed/removed)
- `export-project <tree> <newProjectDir> --new [--vault-name NAME] [--vault-host HOST] [--vault-user DN] [--server NAME --server-context DN] [--catalog DIR] [--designer DIR] [--dry-run] [--json]` — write a **whole new** Designer project for the tree (`docs/designer-new-project.md`, milestones N1+N2+N3). The directory must not exist or be empty, and **its basename is the project name** — it is written into `.project`, `<name>.proj` and `<name>.cproj`, so never rename a project folder by hand afterwards. Writes the descriptors, the three roots, the domain, the vault, the driver set + Library and the package catalog's six stock categories, then everything the tree holds: drivers with channels/filters/policies/resources/GCVs, an `Application_`, a driver icon and a modeler node per driver (the **tree's own icon** when it has one — a custom icon set in Designer survives the round trip — otherwise one derived from the driver type out of a Designer install) (`NProv` for the User Application driver, the shim's or the Designer driver type's application otherwise — a Remote Loader driver is typed from `designer.driver-type`), the UA driver's whole `AppConfig` (`.provisioning`, `.appconfig`, container digests, forms, PRDs) and its entitlements. Packaged items get their package attributes and an `<id>_initial_state.xml` baseline. **`--catalog DIR`** (the git package catalog) additionally writes the project's own package catalog — `IdmCategory_`/`IdmCategoryFolder_`/`IdmPackage_` with its heavy data, nine package folders and one CObject per package item — and the `Idm:InstalledPackages` relations from the driver (type 2), the driver set (type 3) and the vault (type 4) with their back-references; it **refuses the whole run** and lists every package the tree names that the catalog does not hold (`package.fetch` / `package.build` get them), and refuses a package item whose class it does not model rather than guessing. Without `--catalog` no `IdmPackage_` is written and the result note names the packages Designer will not associate. **`--designer DIR`** overrides where *type-derived* driver icons come from (`IDM_DESIGNER`, the `designer` system property, then the platform defaults — the same lookup `form.edit` uses); with no install found, a driver the tree has no icon for gets none and the result says so once (`docs/designer-new-project.md` §7.2c). The vault/server flags are optional and **no vault password is ever written** (`IdentityVaultSavePassword=false`); `--new` never touches an existing project. The driver set gets the name, `DSetContext` and config values the tree records — `DSetCreatePartition`, `DirXML-LogEvents`, the `DirXML-Java*`/`*Trace*`/`LogLimit` settings, `JavaEnvParameters` and `NamedPasswords` are left absent because no tree records them
